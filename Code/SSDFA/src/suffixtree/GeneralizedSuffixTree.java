...
public class GeneralizedSuffixTree {
    ...
    private ArrayList<String> rgStr = new ArrayList<String>();
    ...
    public int[][] allPairSuffixPrefix() {

        int nStr = rgStr.size();
        int[][] score = new int[nStr][nStr];
        int i;
        ArrayList<Stack<Integer>> rgStack = new ArrayList<Stack<Integer>>();
        for (i = 0; i < nStr; i++)
            rgStack.add(new Stack<Integer>());

        dfsAPSP(root, 0, score, rgStack);
    ...
    void dfsAPSP(Node node, int pathDepth, int[][] score, ArrayList<Stack<Integer>> rgStack)
    {
        Map<Character, Edge> edges;
        Collection<Integer> rgTerminals;

        //
        // Identify the strings for which the current node represents a suffix
        //
        rgTerminals = node.getTerminalData();

        edges = node.getEdges();

        for (int j : rgTerminals)
        {
            //
            // Note all the suffix indices corresponding to this node to
            // respective stacks, by storing their lengths.
            //
            rgStack.get(j).push(pathDepth);

            //
            // If the current node represents a complete input string (i.e
            // a suffix that is of the same length of the string itself),
            // then we process the stacks to score the suffix-prefix match
            // of each string with this one.
            //
            if (pathDepth == rgStr.get(j).length()) {
                for (int i = 0; i < rgStack.size(); i++) {
                    Stack<Integer> s = rgStack.get(i);
                    if (!s.isEmpty())
                        score[i][j] = s.peek();
                }
            }
        ...
        {
            dfsAPSP(e.getDest(), pathDepth + e.getLabel().length(), score, rgStack);
        }

        //
        // On the backward path, undo the pushes.
        //
        for (int num : rgTerminals)
        {
            rgStack.get(num).pop();
        }
    }
